#include <iostream>
#include <boost/asio.hpp>
#include <ros/ros.h>
#include <std_msgs/Byte.h>
#include <std_msgs/ByteMultiArray.h>
#include "boost_rs485.hpp"
#include <boost/chrono.hpp>
#include "protocol.hpp"
#include "umba_crc_table.h"

#define CAM_TOPIC_NAME "camera_topic"
#define BAT_CAM_TOPIC_NAME "bat_cam_topic"
#define BAUDRATE 19200

ros::Subscriber bat_cam_sub;
ros::Publisher bat_cam_pub;

class UART_Node
{
public:
    UART_Node(protocol_master::ProtocolMaster& protocol_)
    : m_protocol(protocol_){
        bat_cam_sub = node.subscribe<std_msgs::Byte>(CAM_TOPIC_NAME, 10, &UART_Node::cam_callback,this);
        bat_cam_pub = node.advertise<std_msgs::ByteMultiArray>(BAT_CAM_TOPIC_NAME, 10);
        tp_first = boost::chrono::system_clock::now();
    };

  void UART_process(){
    if(m_protocol.sendCmdRead(0x01, board_data, sizeof(board_data) - sizeof(uint8_t)))
    {
      resvdFromDev |= 128;
      board_data[sizeof(board_data) - sizeof(uint8_t)] = resvdFromDev;
      tp_last = boost::chrono::system_clock::now();
      cam_bat_time = tp_last - tp_first;
      std::cout << "Received BOARD DATA:\n";
      for (size_t i = 0; i < sizeof(board_data); i++)
      {
          printf("[%u]",board_data[i]);
      }
      std::cout << "\nTime since previos msg:" << cam_bat_time << "sec.\n";
      tp_first = boost::chrono::system_clock::now();
      pub_board_data();
      resvdFromDev = 0;
    };
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
  }

private:
    protocol_master::ProtocolMaster& m_protocol;
    ros::NodeHandle node;
    ros::Subscriber bat_cam_sub;
    ros::Publisher bat_cam_pub;
    uint32_t recvd_count_topic = 0;
    uint8_t uart_data[5] = {0};  //-->to board
    uint8_t board_data[8] = {0}; //<--from board
    boost::chrono::system_clock::time_point tp_first;// = boost::chrono::system_clock::now();
    boost::chrono::system_clock::time_point tp_last;
    boost::chrono::duration<double> cam_bat_time;
    uint8_t resvdFromDev = 0;

void pub_board_data() //create and pub ros message 
{
    std_msgs::ByteMultiArray boardDataMsg;
    boardDataMsg.layout.dim.push_back(std_msgs::MultiArrayDimension());
    boardDataMsg.layout.dim[0].size = 1;
    boardDataMsg.layout.dim[0].stride = sizeof(board_data);
    boardDataMsg.data.clear();

    std::cout << "SEND TO TOPIC bat_cam_topic: ";
    for (size_t i = 0; i < sizeof(board_data); i++)
    {
        boardDataMsg.data.push_back(board_data[i]);
        printf("[%u]", board_data[i]);
    }
    bat_cam_pub.publish(boardDataMsg);
}

void cam_callback(const std_msgs::Byte::ConstPtr& camStatus)
{
    recvd_count_topic++;
    uint8_t cam_status = camStatus->data;

    std::cout << "recvd_count_topic = " << recvd_count_topic << std::endl;
    std::cout << "\nCAMERA STATUS ";
    printf("%u\n", cam_status);
    if (m_protocol.sendCmdReadRead(0x01, &cam_status, sizeof(uint8_t), board_data,sizeof(board_data) - sizeof(uint8_t))){
      resvdFromDev |= 128;
      board_data[sizeof(board_data) - sizeof(uint8_t)] = resvdFromDev;
    }
    pub_board_data();
    resvdFromDev = 0;
    std::cout << endl;
}
};

int main(int argc, char** argv)
{
    std::string devPort = "";
    std::string baudrate = "19200";
	if(argc == 3) {
		devPort = argv[1];
        baudrate = argv[2];
	} else {
		std::cerr << "[program_name][/dev/ttyS(0, 1...)][baudrate]" << std::endl;
		return -1;
	}
	try{
		std::cout << "\nUART Node is running!\n" << "Baud rate: " << baudrate << ", Port: /dev/ttyS" << devPort << '\n';
		ros::init(argc, argv, "uart_node");
        boost_rs485::Boost_RS485_Master boostRS485_transp("/dev/ttyUSB" + devPort, (uint32_t)std::stoi(baudrate));
        protocol_master::ProtocolMaster boostRS485_prot_master(boostRS485_transp);
        UART_Node uartNode(boostRS485_prot_master);
        while(ros::ok()){
            uartNode.UART_process();
            ros::spinOnce();
        }
	} catch (std::exception e){
		std::cerr << "Exeption: " << e.what() << std::endl;
	}
  return 0;
};