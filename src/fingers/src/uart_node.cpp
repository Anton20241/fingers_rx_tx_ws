...
public:
  ...
  void UART_process(){
    if(m_protocol.sendCmdRead(0x01, from_board_data, sizeof(from_board_data)))
    {
      resvdFromDev |= 128;
      tp_last = boost::chrono::system_clock::now();
      cam_bat_time = tp_last - tp_first;
      std::cout << "Received BOARD DATA:\n";
      for (size_t i = 0; i < sizeof(from_board_data); i++)
      {
...
void pub_board_data() //create and pub ros message 
{
    ...
    toBatCamTopicMsg.layout.dim[0].stride = sizeof(to_bat_cam_topic);
    toBatCamTopicMsg.data.clear();

    std::cout << "SEND TO TOPIC bat_cam_topic: ";
    for (size_t i = 0; i < sizeof(to_bat_cam_topic); i++)
    {
...
void cam_callback(const std_msgs::ByteMultiArray::ConstPtr& camStatus)
{
    ...
    if (m_protocol.sendCmdWrite(0x01, toRelaySet, sizeof(toRelaySet))){
      resvdFromDev |= 128;
    }
    if (m_protocol.sendCmdReadRead(0x01, &cam_status, sizeof(uint8_t), from_board_data,
        sizeof(from_board_data) - sizeof(uint8_t))){
      resvdFromDev |= 128;
    }
    pub_board_data();
    std::cout << endl;
